...
describe('SearchBar integration test', () => {
  it('should flow', async () => {
    ...
    const menuItemsShouldMatchSecondAPIcall = () => {
      expect(wrapper.find('.dropdown-header').at(0).text()).toBe(secondAPICall[0].category);
      expect(wrapper.find('.dropdown-item').at(0).text()).toBe(secondAPICall[0].label);
      expect(wrapper.find('.dropdown-header').at(1).text()).toBe(secondAPICall[1].category);
      expect(wrapper.find('.dropdown-item').at(1).text()).toBe(secondAPICall[1].label);
    };
